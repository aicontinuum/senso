...
| Ingest as a writer | Keeps auth and validation; 6-hour lower bound on `time`; dedup on `deduplicationId`; no threshold logic. Lookup, insert and gateway stamp retry three times; a reading still lost answers 503 and writes an `ingest` row to `job_runs`. | **Live** (phase 4, retries 2026-09-13). |
