...
- [x] ~~**Customers could not see their "stopped reporting" alerts.**~~ **FOUND AND CLOSED 2026-09-25 by `20260927_groups.sql`** *(added 2026-09-25)* — the live `alert_logs_select_own` policy reached an alert only through `alert_config_id`, so a `sensor_offline` row (which has none) was hidden from the customer by row-level security: emailed about, then absent from the Alerts page. The groups migration rewrites the rule over `coalesce(sensor_id, config's sensor_id)`; `supabase/tests/groups` proves a member sees both kinds.
...
- [x] ~~**`customers` UPDATE policy too broad**~~ **CLOSED 2026-09-26 by `20261001_customer_write_scope.sql`.** Original finding: **`customers` UPDATE policy too broad** — own-row only, but a customer can `update({billing_status, email, name})` directly from the browser (self-service billing/identity tampering). Restrict to `contact_name`/`phone`/`alert_recipients`/`timezone` via column grants or a SECURITY DEFINER function. (This is part of the RLS Critical item.)
...
- [x] ~~**Threshold validation on `/api/sensors/[id]`**~~ **CLOSED 2026-09-26** — `validateThresholds()` in `lib/thresholds.ts`, checked before any write. Original finding: **Threshold validation on `/api/sensors/[id]`** — `NaN >= NaN` is false, so a non-numeric min/max passes and writes `null` thresholds (disables alerting); no bounds check. Validate numeric + sane range + min<max server-side.
...
- [x] ~~**Customer-create password/email not validated server-side**~~ **CLOSED 2026-09-26** — `POST /api/customers` checks name, email, password length and the group flag through the shared validators. — `/api/customers` forwards `password` to `createUser` with no strength check (password-change enforces ≥8) and no server email regex. Add both.
...
- [x] ~~**CSV formula injection + broken quoting**~~ **CLOSED 2026-09-26** — `lib/csv.ts`. Original finding: **CSV formula injection + broken quoting** — `reports/ReportClient.tsx` export doesn't double embedded `"` and has no neutralizing prefix for `= + - @`. Vector is admin-set names (lower likelihood), but escape quotes and prefix risky cells.
...
- [x] ~~**Raw Supabase `error.message` returned to clients**~~ **CLOSED 2026-09-26** — every admin route now answers through `failureResponse`: bad input is a 400 in words, a database rule a 409, anything else a logged generic 500; the customer routes were already generic. (admin + customer routes) — leaks schema/constraint details. Return generic messages; log details server-side.
...
- [x] ~~**No security headers**~~ **CLOSED 2026-09-26** — `packages/security`: fixed headers from `next.config.ts`, nonce CSP from `proxy.ts`, both apps.
