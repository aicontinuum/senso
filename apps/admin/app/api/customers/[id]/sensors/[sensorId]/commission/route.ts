...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string; sensorId: string }> }) {
  ...
  try {
    ...
    const { data: sensor, error: readError } = await ctx.admin
      .from('sensors')
    ...
    const { error: updateError } = await ctx.admin
      .from('sensors').update({ commissioned_at: commissionedAt }).eq('id', sensorId);
    ...
    const { error: auditError } = await ctx.admin.from('sensor_commissioning_events').insert({
      sensor_id: sensorId,
      action: 'commissioned',
      commissioned_at: commissionedAt,
      actor_id: ctx.actorId,
    });
    if (auditError) console.error('[api] commission audit write failed', { sensorId, error: auditError });

    return NextResponse.json({ success: true, commissionedAt });
  } catch (error) {
    return failureResponse('commission sensor', error);
  }
}
