...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const { error } = await admin.from('billing_settings').update({
      ...
      months_charged_6: requireCount(body.monthsCharged6, 'months charged on a 6-month term'),
      months_charged_12: requireCount(body.monthsCharged12, 'months charged on a 12-month term'),
