...
//
// Deliberately **fail-closed**: if `CHIRPSTACK_INGEST_SECRET` is unset, every
// request is rejected rather than waved through. The retired Pi kit's per-gateway
// check was rollout-safe (no secret = allowed) so that deploying auth before
// provisioning gateways couldn't break ingestion, which left an unauthenticated
// write path into the compliance record. There's no rollout problem here: one
// secret, set once, before the integration is switched on.
//
