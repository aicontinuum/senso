...
type MemberRow = { member_id: string; customers: { id: string; name: string; email: string } | null };
type GroupRow = { group_id: string; customers: { id: string; name: string } | null };

/** The accounts an owner login reads, by name. */
export async function loadGroupMembers(admin: Admin, groupId: string): Promise<GroupMember[]> {
  const { data, error } = await admin
    .from('customer_group_members')
    .select('member_id, customers!customer_group_members_member_id_fkey (id, name, email)')
    .eq('group_id', groupId)
    .order('created_at');
  if (error) throw new Error(`customer_group_members: ${error.message}`);
  return (data as unknown as MemberRow[]).flatMap(r => (r.customers ? [r.customers] : []));
}
...
export async function loadUnlinkedAccounts(admin: Admin): Promise<GroupMember[]> {
