...
// in the flag sent with the request and the words around the fields.
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="rounded-lg border bg-card shadow-sm px-6 py-10 text-center space-y-3">
          ...
          <p className="font-semibold text-lg">{isGroup ? 'Group account created' : 'Customer created'}</p>
      ...
      <div className="rounded-lg border bg-card shadow-sm">
        <div className="border-b px-6 py-4">
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {isGroup
              ? 'Creates an owner login that sees every account linked under it and changes nothing. It has no gateways or sensors of its own; link the member accounts on its page once it is created.'
            ...
            {FIELDS.map(f => (
              <div key={f.key}>
                <label className="block text-sm font-medium mb-1">
                  {isGroup && f.key === 'name' ? 'Group Name' : f.label}{f.required && <span className="text-alert-text ml-0.5">*</span>}
                </label>
                <input
                  type={f.type ?? 'text'}
                  value={form[f.key]}
                  onChange={set(f.key)}
                  placeholder={isGroup && f.key === 'name' ? 'e.g. Al Noor Group' : f.placeholder}
