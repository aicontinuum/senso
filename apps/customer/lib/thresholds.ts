...
// ── Input from the sensor page ──────────────────────────────────────────────
// The bounds match what ingest will store (apps/admin/app/api/ingest): a
...
export function validateThresholds(minTemp: unknown, maxTemp: unknown): { ok: true; min: number; max: number } | { ok: false; error: string } {
