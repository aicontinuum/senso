...
echo "1/7  Installing Wi-Fi power-save drop-in…"
install -m 644 "$DIR/wifi-powersave-off.conf" /etc/NetworkManager/conf.d/wifi-powersave-off.conf

echo "2/7  Enabling autoconnect with unlimited retries…"
...
echo "3/7  Installing the connectivity watchdog…"
...
echo "4/7  Installing the heartbeat…"
...
echo "5/7  Installing the LoRa forwarder (store-and-forward)…"
...
echo "6/7  Enabling the hardware watchdog (freeze recovery)…"
install -d -m 755 /etc/systemd/system.conf.d
install -m 644 "$DIR/watchdog.conf" /etc/systemd/system.conf.d/senso-watchdog.conf
# Ensure the hardware watchdog device is enabled at boot.
BOOTCFG=/boot/firmware/config.txt
[ -f "$BOOTCFG" ] || BOOTCFG=/boot/config.txt
if [ -f "$BOOTCFG" ] && ! grep -q '^dtparam=watchdog=on' "$BOOTCFG"; then
  echo 'dtparam=watchdog=on' >> "$BOOTCFG"
  echo "     enabled dtparam=watchdog=on in $BOOTCFG"
fi
...
echo
echo "Done. Reboot to apply the Wi-Fi power-save setting and hardware watchdog:  sudo reboot"
